...
# PyTorch Dataset for COCO-Text v2
class COCOTextDataset(Dataset):
    def __init__(self, img_dir, ann_file, img_size=512, num_rays=20, split='train'):
        self.img_dir = img_dir
        self.img_size = img_size
        self.num_rays = num_rays
        self.split = split
        
        print(f"Loading COCO-Text annotations from {ann_file} for sequence '{split}'...")
        ...
        self.img_to_anns = {img_id: [] for img_id in self.imgs.keys()}
            ...
            if img_id in self.img_to_anns:
                if 'mask' in ann and isinstance(ann['mask'], list) and len(ann['mask']) > 0:
                    ...
                    if isinstance(mask[0], list):
                        mask = mask[0]
                    # Since we previously used upsample_coco_polygon to exactly 14 vertices (28 coordinates)
                    if len(mask) == 28:
                        self.img_to_anns[img_id].append(mask)
    ...
    def parse_coco_mask(self, masks, orig_w, orig_h):
        polygons = []
        for mask in masks:
            x_vals = mask[0::2]
            y_vals = mask[1::2]
    ...
    def __getitem__(self, idx):
        ...
        if img is None:
            # Fallback for missing images gracefully via zeros, dataset could drop it but collate requires consistent shape
        ...
        # Load annotations
        masks = self.img_to_anns[img_id]
        polygons = self.parse_coco_mask(masks, orig_w, orig_h)
...
def train():
    ...
    checkpoint = r"D:\computer vision\Scene-total-text-detection\src\fcenet_radial_head_finetuned.pth" 
    
    coco_img_dir = r"D:\computer vision\Scene-total-text-detection\dataset\cocotext\images"
    coco_ann_file = r"D:\computer vision\Scene-total-text-detection\dataset\cocotext\cocotext.v2_14pts.json"
    ...
    # 3. Load Backbone/Neck Weights from Finetuned
    print(f"Loading Checkpoint: {checkpoint}")
    ckpt = torch.load(checkpoint, map_location="cpu")
    # if it's purely a state dict (like how you saved last time), load it directly
    ...
    train_dataset = COCOTextDataset(img_dir=coco_img_dir, ann_file=coco_ann_file, 
                                     img_size=img_size, num_rays=num_rays, split='train')
    
    # We will subset it a bit or just use num_workers if COCO is too huge
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, 
                              num_workers=4, pin_memory=True, collate_fn=collate_fn)

    print("Initializing Validation Dataset...")
    val_dataset = COCOTextDataset(img_dir=coco_img_dir, ann_file=coco_ann_file, 
                                   img_size=img_size, num_rays=num_rays, split='val')
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, 
                            num_workers=4, pin_memory=True, collate_fn=collate_fn)

    # 5. Optimizer and Loss Function
    optimizer = torch.optim.AdamW(model.det_head.parameters(), lr=1e-4) # You could also lower the learning rate if finetuning again
    ...
    for epoch in range(epochs):
        ...
        precision = correct_preds / max(total_gt_boxes, 1) 
        recall = precision 
    ...
    torch.save(model.state_dict(), "fcenet_radial_head_coco_finetuned.pth")
